...
public class VelocityClassLoaderTest extends TestCase {
   ...
   * an {@code @AutoCursor} class. If Velocity loads its managers using the context class loader,
  ...
  public static class RunInClassLoader implements Runnable {
    ...
    public void run() {
      String source = Joiner.on('\n').join(ImmutableList.of(
          "package foo.bar;",
          "import " + AutoCursor.class.getName() + ";",
          "@AutoCursor abstract class Test {",
          "  abstract int baz();",
          "  static Test create(int baz) {",
          "    return new AutoCursor_Test(baz);",
          "  }",
          "}"));
