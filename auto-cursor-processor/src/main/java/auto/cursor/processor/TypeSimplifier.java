...
final class TypeSimplifier {
   ...
   * the package containing the AutoCursor class, provided that the result refers to the type
  ...
  // If we have @AutoCursor abstract class Foo<T extends SomeClass> then this method will
  ...
  // If we have @AutoCursor abstract class Foo<T extends Something> then the subclass will be
  // final class AutoCursor_Foo<T extends Something> extends Foo<T>.
