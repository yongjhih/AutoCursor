 ...
 * the class with the {@code @AutoCursor} annotation, and we do just enough parsing of that code to
 ...
 * {@code @AutoCursor} constructor calls like {@code new AutoCursor_Foo(...)} suddenly start being
...
class EclipseHack {
   ...
   * has no effect.
   */
  void reorderProperties(List<AutoCursorProcessor.Property> properties) {
    ...
    while (index < properties.size()) {
      ...
      List<AutoCursorProcessor.Property> subList = properties.subList(index, nextIndex);
