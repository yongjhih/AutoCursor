...
public class AutoCursorProcessor extends AbstractProcessor {
  public AutoCursorProcessor() {
  }

  @Override
  public Set<String> getSupportedAnnotationTypes() {
    return ImmutableSet.of(AutoCursor.class.getName());
  }
  ...
  /**
   * Qualified names of {@code @AutoCursor} classes that we attempted to process but had to abandon
  ...
  public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
    ...
    if (roundEnv.processingOver()) {
      ...
      // any new instances of @AutoCursor; and we can't have any new types that are the reason a type
      // was in deferredTypes.
      for (TypeElement type : deferredTypes) {
        errorReporter.reportError("Did not generate @AutoCursor class for " + type.getQualifiedName()
            + " because it references undefined types", type);
      }
      return false;
    }
    Collection<? extends Element> annotatedElements =
        roundEnv.getElementsAnnotatedWith(AutoCursor.class);
    ...
    for (TypeElement type : types) {
      try {
        ...
        // again and adding it back to the list). We save the name of the @AutoCursor type rather
        ...
        errorReporter.reportError("@AutoCursor processor threw an exception: " + trace, type);
      }
    }
  ...
  private String generatedSubclassName(TypeElement type) {
    return generatedClassName(type, "AutoCursor_");
  }

  /**
   * A property of an {@code @AutoCursor} class, defined by one of its abstract methods.
  ...
  public static class Property {
    ...
    /**
     * Returns the name of the getter method for this property as defined by the {@code @AutoCursor}
  ...
  private void processType(TypeElement type) {
    AutoCursor autoValue = type.getAnnotation(AutoCursor.class);
    if (autoValue == null) {
      // This shouldn't happen unless the compilation environment is buggy,
      // but it has happened in the past and can crash the compiler.
      errorReporter.abortWithError("annotation processor for @AutoCursor was invoked with a type"
          + " that does not have that annotation; this is probably a compiler bug", type);
    }
    if (type.getKind() != ElementKind.CLASS) {
      errorReporter.abortWithError(
          "@" + AutoCursor.class.getName() + " only applies to classes", type);
    }
    if (ancestorIsAutoCursor(type)) {
      errorReporter.abortWithError("One @AutoCursor class may not extend another", type);
    }
    if (implementsAnnotation(type)) {
      errorReporter.abortWithError("@AutoCursor may not be used to implement an annotation"
          + " interface; try using @AutoAnnotation instead", type);
    }
    AutoCursorTemplateVars vars = new AutoCursorTemplateVars();
    ...
    for (Map.Entry<ExecutableElement, String> entry : methodToIdentifier.entrySet()) {
      if (!seen.add(entry.getValue())) {
        errorReporter.reportError(
            "More than one @AutoCursor property called " + entry.getValue(), entry.getKey());
      }
    }
  }
  ...
  private static void determineObjectMethodsToGenerate(
      List<ExecutableElement> methods, AutoCursorTemplateVars vars) {
      ...
      if (method.getModifiers().contains(Modifier.ABSTRACT)
        ...
        if (method.getParameters().isEmpty() && method.getReturnType().getKind() != TypeKind.VOID) {
          if (isReferenceArrayType(method.getReturnType())) {
            errorReporter.reportError("An @AutoCursor class cannot define an array-valued property"
          ...
          // that overrides it. This shows up in AutoCursorTest.LukesBase for example.
          errorReporter.reportWarning("@AutoCursor classes cannot have abstract methods other than"
  ...
  private boolean ancestorIsAutoCursor(TypeElement type) {
    while (true) {
      ...
      if (parentElement.getAnnotation(AutoCursor.class) != null) {
        return true;
      }
      type = parentElement;
    }
  ...
  // The @AutoCursor type, with a ? for every type.
  // If we have @AutoCursor abstract class Foo<T extends Something> then this method will return
  // just <?>.
