...
class BuilderSpec {
  ...
  /**
   * Determines if the {@code @AutoCursor} class for this instance has a correct nested
   * {@code @AutoCursor.Builder} class or interface and return a representation of it in an
   * {@code Optional} if so.
   */
  Optional<Builder> getBuilder() {
    ...
    for (TypeElement containedClass : ElementFilter.typesIn(autoValueClass.getEnclosedElements())) {
      if (MoreElements.isAnnotationPresent(containedClass, AutoCursor.Builder.class)) {
        if (!CLASS_OR_INTERFACE.contains(containedClass.getKind())) {
          errorReporter.reportError(
              "@AutoCursor.Builder can only apply to a class or an interface", containedClass);
    ...
    for (ExecutableElement containedMethod :
      ...
      if (MoreElements.isAnnotationPresent(containedMethod, AutoCursor.Validate.class)) {
        if (containedMethod.getModifiers().contains(Modifier.STATIC)) {
          errorReporter.reportError(
              "@AutoCursor.Validate cannot apply to a static method", containedMethod);
        } else if (!containedMethod.getParameters().isEmpty()) {
          errorReporter.reportError(
              "@AutoCursor.Validate method must not have parameters", containedMethod);
        } else if (containedMethod.getReturnType().getKind() != TypeKind.VOID) {
          errorReporter.reportError(
              "Return type of @AutoCursor.Validate method must be void", containedMethod);
        } else if (validateMethod.isPresent()) {
          errorReporter.reportError(
              "There can only be one @AutoCursor.Validate method", containedMethod);
        } else {
  ...
  /**
   * Representation of an {@code AutoCursor.Builder} class or interface.
   */
  class Builder {
  ...
  /**
   * Returns a representation of the given {@code @AutoCursor.Builder} class or interface. If the
  ...
  private Optional<Builder> builderFrom(
    ...
    // We require the builder to have the same type parameters as the @AutoCursor class, meaning the
    ...
    // builderTypeElement. A build method has no arguments and returns the @AutoCursor class.
